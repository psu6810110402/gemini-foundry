...
export async function POST(req: Request) {
  try {
    ...
    // 1. Validate Input
    const validation = MVPBlueprintSchema.safeParse(body);
    ...
    // 2. Prepare Prompt
    const prompt = `${PERSONAS.MVP}\n\nStartup Idea:\n${idea}`;

    const result = await generateStructuredContent(prompt, MVPBlueprintOutputSchema);
    ...
    console.error("Error in MVP blueprint:", error);
